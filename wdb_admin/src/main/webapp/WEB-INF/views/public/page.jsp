...
<c:choose>
	<c:when test="${page.total>0 }">
		<div class="m-page">
			<c:if test="${page.hasPrev }">
				<a class="prev" href="${param.url}<c:if test="${param.paramlist!=\"\" }">?${param.paramlist }&</c:if><c:if test="${param.paramlist==\"\" }">?</c:if>page=${page.page-1 } ">上一页</a>
			</c:if>
			...
			<c:if test="${page.hasNext }">
				<a class="next" href="${param.url}<c:if test="${param.paramlist!=\"\" }">?${param.paramlist }&</c:if><c:if test="${param.paramlist==\"\" }">?</c:if>page=${page.page+1 } ">下一页</a>
			</c:if>
			&nbsp;&nbsp;&nbsp;&nbsp;共${page.maxPage }页
	...
	<c:otherwise>
		...
		<div class="muted f14 tc">
			${!(empty param.tip) ? param.tip:"抱歉,没有找到相关信息"}
		</div>
	</c:otherwise>
</c:choose>
