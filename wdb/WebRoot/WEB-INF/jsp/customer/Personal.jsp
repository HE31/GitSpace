...
function open3()
{
Dialog.confirm('&nbsp;&nbsp;温馨提示：您确定要逾期吗？<br>&nbsp;&nbsp;<font color="#FF7F00"><strong>逾期将增加相应还款金额！</font></strong>',function open2()
{
	...
	diag.Height = 185;
	diag.Title = "申请逾期";
	diag.URL = "${pageContext.request.contextPath }/order/overdue.action?ordernumber="+ordernumber.value+" ";
	diag.show();
...
.buttonStyle{width:64px;height:22px;line-height:22px;color:#369;text-align:center;background:url(../images/tanchu_icon/buticon.gif) no-repeat left top;border:0;font-size:12px "Microsoft Yahei";}
...
﻿             <div class="fl persMenu">
                <ul class="u-sidebar">
                    <li>
                        <a class="current"  href="${pageContext.request.contextPath }/customer/Personal.action">
...
<div class="wd945 fs14 cc0"><p class="fl">信用账单</p></div> 
										...
										<c:choose><c:when test="${empty wdbOrdersQueryVo.get(0).getWdbStagesCustom().getMoney()  }">
										￥0.00</c:when>
										<c:otherwise>
											￥${wdbOrdersQueryVo.get(0).getWdbStagesCustom().getMoney() }
											</c:otherwise>
											</c:choose>
                    ...
                    <div class="proItem2">
                    	...
                    	<table class="centTab1 fs14" cellpadding="0" cellspacing="0" border="0">
                            <tbody><tr>
                             <th width="120">订单号</th>
                                ...
                                <th width="100" class="myTextRight">金额（元）</th>
                                <th width="100">交易状态</th>
                                <th width="100">操作</th>
                                
                            </tr>
                            <c:choose>
                            <c:when test="${empty wdbOrdersQueryVo}">
                       <tr>    <td></td><td></td><td><p style="letter-spacing:20px;">暂无相关订单</p></td></tr>
	                        ...
	                        <td>${order.getOrdernumber() } </td>
	                            <td>${orderdate } </td>
	                            <td>
	                            <p class="mt10" style="height:50px" title=${order.getWdbProductsCustom().getProductname() } data-title=${order.getWdbProductsCustom().getProductname() }>
	${order.getWdbProductsCustom().getProductname() }
	                       		</p>	 
	                            </td>
	                            <td class="myTextRight">${order.getWdbStagesCustom().getMoney() }</td>
	                            <td>
               					您已还款至第${order.getWdbStagesCustom().getTime() } 期 ,本期截止还款日期：
               					<fmt:formatDate value='${order.getWdbStagesCustom().getEnddate() }' pattern='yyyy-MM-dd'/>
               					,共${order.getTimes() }期
	                            </td>
	                           <c:choose>
	                           ...
	                           <td>送货中</td></c:when>
	                           <c:otherwise>
	                            <td><a style="color:#5ca8fd" href="${pageContext.request.contextPath }/order/repayment.action?ordernumber=${order.getOrdernumber() }">我要还款</a><br>
	                            <c:choose>
	                            <c:when test="${order.getWdbStagesCustom().getRepaystatus() eq -1 }">
	                            <a href="#" onclick="" >已申请逾期</a>
	                            </c:when>
	                            <c:otherwise>
	                            <input type="hidden" id="ordernumber" value="${order.getOrdernumber() }">
	                            <a href="#" onclick="open3()" id="b">申请逾期</a> 
	                            </c:otherwise>
	                            </c:choose> 
	                            </td>
	                            </c:otherwise>
